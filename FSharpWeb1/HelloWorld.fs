...
// The feed downloading code was adapted from:
...
type HelloWorld() =
  static member private DownloadFeeds() =
    ...
    |> Map.iter(fun key value -> 
        ...
        rss.Items <- rss.Items.Union(feed.Items)
  ...
  static member private DownloadFeed(url:string) : SyndicationFeed =
    using(new WebClient()) 
      (fun client -> 
        try
          let stream = async { return! client.OpenReadTaskAsync(url) |> Async.AwaitTask } |> Async.StartAsTask
          SyndicationFeed.Load(XmlReader.Create(stream.Result))
        with
          | _ -> 
              new SyndicationFeed()) 
  ...
  static member Handler(ctx:HttpContext) =      
    ...
    // It'd be nice if we could do an asynchronous Response.Write but I'm not sure that's possible
    // We'd need a better way to handle the results while they are downloading. I tried a bunch of 
    // shit but am still thoroughly confused with F# Async Workflows!

    HelloWorld.DownloadFeeds() 
    |> Seq.iter (fun i -> feedHtmlBuilder.Append(String.Format("<a href='{0}' target='_blank'>{1}</a><br>", i.Links.[0].Uri, i.Title.Text)) |> ignore)
